...
<body>
	...
	<form:form id="inputForm" modelAttribute="article" action="${ctx}/cms/article/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<span class="help-inline">数值越大排序越靠前，过期时间可为空，过期后取消置顶。</span>
			  	...
			  	&nbsp;过期时间：
				<form:input path="weightDate"  readonly="true" maxlength="20" class="input-small Wdate"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:true});"/>
				<span class="help-inline">数值越大排序越靠前，过期时间可为空，过期后取消置顶。</span>
				-->
			</div>
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<tags:ckfinder input="image" type="images" uploadPath="/cms/article" selectMultiple="false"/>
			 	-->
			 	<form:input type="hidden" path="image"  value="${article.imageSrc}" />
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="createDate"  readonly="true" maxlength="20" class="input-medium Wdate"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
			</div>
			-->
		</div>
